...
public class BulletinDAO extends DAO<Bulletin> {
	...
	public  Bulletin find(int id)
	{
		...
		try {
			...
			if(res.first())
			{
				bulletin=new Bulletin(res.getInt("id"),res.getString("appreciation"),res.getInt("fk_eleve"));
				
				TrimestreDAO trimestre = new TrimestreDAO(this.conect);
				
				res=this.conect.executeQuerry("SELECT trimestre.numero FROM trimestre INNER JOIN bulletin ON trimestre.id=bulletin.fk_trimestre "
						+ "WHERE bulletin.id="+id);
				if(res.first()) {
					
					bulletin.set_trimestre(res.getString("numero"));
